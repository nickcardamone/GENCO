#Title: 1.Connect to Dropbox and downloads formulary files
...
# of orally administered drugs in each year after NME approva
...
# Notes:
# * I had to manually unzip all of the zip files and extract the .txt files within them because 
# for an unknown reason when R was unzipping and reading the files, it was omitting portions of the data.
...
drop_auth_RT <- function (new_user = FALSE, key = "TOKEN KEY", secret = "TOKEN SECRET", cache = TRUE, rdstoken = NA) 
{
...
download_function <- function(match_item) {
  drop_download(match_item$metadata$path_lower, local_path = "YOUR PATH", overwrite = TRUE)
}
