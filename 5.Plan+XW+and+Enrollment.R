...
#8. Create a “conversion date” variable – the period at which NDCs associated with a generic application number appear in 10% of the formularies this basket of generic NDCs is ever going to appear in.
...
combined_plan_data <- combined_plan_data %>% 
  ...
  mutate(plan_id = str_pad(plan_id, 3, pad = "0")) %>% distinct()

#write.csv(combined_plan_data, "combined_plan_data_n=134198-10-31-2024.csv")


# Load Plan-Formulary Data}
combined_plan_data <- read.csv("combined_plan_data_n=134198-10-31-2024.csv")
...
combined_plan_data <- combined_plan_data %>% 
  ...
  group_by(formulary_id, contract_id, plan_id, segment_id, date)
...
#write.csv(all_data, "part3-all-enrollment-data-1-7-2025.csv")


enrollment <- read.csv("part3-all-enrollment-data-1-7-2025.csv")



## Clean Enrollment Data & Create indicator for 6 month period
enrollment <- data.frame(contract_id = enrollment$`contract.number`,
                         plan_id = enrollment$`plan.id`,
                         enrollment = enrollment$enrollment,
                         partd = enrollment$`offers.part.d`,
                         date = as.Date(enrollment$date))
...
## Join Contract-Plan Crosswalk to Enrollment Data
...
# Join enrollment data to plan data
...
## Remove leading 000 from the formulary_id from the enrollment data.}
enrollmentlj$formulary_id <- sub("^000", "", enrollmentlj$formulary_id)

## Save enrollment data locally
write_parquet(enrollmentlj, 'parquet/plan_enrollment.parquet')
